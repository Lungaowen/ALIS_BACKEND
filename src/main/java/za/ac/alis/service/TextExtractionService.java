...
public class TextExtractionService {
    ...
    public TextExtractionService(FirebaseStorageService firebaseStorageService) {
        this.firebaseStorageService = firebaseStorageService;
    }

    /**
     * Main method used by AI Pipeline - prefers direct byte download from Firebase
     */
    public String extractText(Long documentId, String objectPath, String mimeType) throws Exception {
        log.info("Starting text extraction for Document ID={}, MIME={}", documentId, mimeType);
        try {
            byte[] fileBytes = firebaseStorageService.downloadFile(objectPath);
            log.debug("Successfully downloaded {} bytes from Firebase", fileBytes.length);
            return extractTextFromBytes(fileBytes, mimeType);
        } catch (Exception e) {
            log.error("Failed to download from Firebase for Document ID={}", documentId, e);
            throw new RuntimeException("Text extraction failed for document " + documentId, e);
        }
    }
